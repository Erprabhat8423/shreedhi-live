        ...
        try:
            ...
            if(error_count > 0):
                ...
                if request.POST['last_user_id'] != '':
                    ...
                    if request.POST['marriage_date']:
                        basic.marriage_date = datetime.strptime(request.POST['marriage_date'], '%d/%m/%Y').strftime('%Y-%m-%d')
    ...
    if request.method == "POST":
        try:
            ...
            if(error_count > 0):
                ...
                user_basic_details.gumasta_number       = request.POST['gumasta_number']
                if request.POST['gumasta_expiry_date']:
                        user_basic_details.gumasta_expiry_date   = datetime.strptime(request.POST['gumasta_expiry_date'], '%d/%m/%Y').strftime('%Y-%m-%d')
                ...
                user_basic_details.tcs_applicable       = "0"
                ...
                user_basic_details.tcs_value            = 0.0
                ...
                if SpUserLedger.objects.filter(user_id = request.POST['last_user_id']).exists():
                    # check_balance = SpBasicDetails.objects.get(user_id=request.POST['last_user_id'])
                    # math.isclose(a + b, c)
                    pass
                else:
                    ledger = SpUserLedger()
...
def editUserOfficalDetail(request):
    ...
    #     roles           = SpRoles.objects.filter(department_id=3).filter(id__in=[8,9])
    # else:
    roles           = SpRoles.objects.filter(department_id__icontains=user_details.department_id)
    towns           = SpTowns.objects.filter(zone_id=user_area_allocations.zone_id)
    if user_area_allocations:
        vehicle         = SpVehicles.objects.filter(route_id = user_area_allocations.route_id)
    else:
...
def mapProductToUser(user_id):
    ...
    product_variants = SpProductVariants.objects.filter(production_unit_id = Production_unit_id) 
    if len(product_variants):
        # SpUserProductVariants.objects.filter(user_id=user_id).delete()
        for product_variant in product_variants:
            if not SpUserProductVariants.objects.filter(user_id=user_id,product_variant_id = product_variant.id).exists():
...
@login_required
def updateUserVariantLimit(request):
    response = {}
    if request.method == "POST":
        try:
            id              = request.POST.get('id')
...
def clearDeviceId(request):
    ...
    if request.method == "GET":
        try:
            ...
            # AuthtokenToken.objects.filter(user_id=id).delete()
            # device_id = getModelColumnById(SpUsers, id, 'device_id')
            ...
            if type == '0':    
                heading     = 'Location Cleared'
                activity    = 'Location Cleared of '+emp_name+' by '+user_name+' at '+datetime.now().strftime('%d/%m/%Y | %I:%M %p') 
            else:
                heading     = 'Device Id Cleared'
                activity    = 'Device Id of '+emp_name+' has been cleared by '+user_name+' at '+datetime.now().strftime('%d/%m/%Y | %I:%M %p') 
            saveActivity('Users Management', 'Users', heading, activity, request.user.id, user_name, 'add.png', '1', 'web.png')
            response['error'] = False
            response['message'] = "Record has been updated successfully."
        except ObjectDoesNotExist:
