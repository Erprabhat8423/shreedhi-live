...
def index(request):
    ...
    if last_free_scheme:
        ...
        scheme_free_users = list(SpUserSchemes.objects.raw(''' SELECT sp_user_schemes.*, sp_users.id as user_id,sp_users.first_name as first_name,sp_users.middle_name as middle_name,sp_users.last_name as last_name,sp_users.store_name as store_name,sp_users.emp_sap_id as emp_sap_id,
    ...
    if last_flat_incentive_scheme:
        scheme_flat_incentive_users = list(SpUserFlatSchemes.objects.raw(''' SELECT sp_user_flat_schemes.*, sp_users.id as user_id,sp_users.first_name as first_name,sp_users.middle_name as middle_name,sp_users.last_name as last_name,sp_users.store_name as store_name,sp_users.emp_sap_id as emp_sap_id,
...
def latestFlatIncentiveUsers(request):
    ...
    if latest_scheme :
        ...
        users = SpUserFlatSchemes.objects.raw(''' SELECT sp_user_flat_schemes.*, sp_users.id as user_id,sp_users.first_name as first_name,sp_users.middle_name as middle_name,sp_users.last_name as last_name,sp_users.store_name as store_name,sp_users.emp_sap_id as emp_sap_id,
        sp_user_area_allocations.town_name,sp_user_area_allocations.route_name,sp_user_area_allocations.state_name FROM sp_user_flat_schemes
        LEFT JOIN sp_users on sp_users.id = sp_user_flat_schemes.user_id
        LEFT JOIN sp_user_area_allocations on sp_user_area_allocations.user_id = sp_user_flat_schemes.user_id
        WHERE sp_user_flat_schemes.scheme_id = %s
...
def latestBulkpackIncentiveUsers(request):
    ...
    if latest_scheme :
        ...
        users = SpUserBulkpackSchemes.objects.raw(''' SELECT sp_user_bulkpack_schemes.*, sp_users.id as user_id,sp_users.first_name as first_name,sp_users.middle_name as middle_name,sp_users.last_name as last_name,sp_users.store_name as store_name,sp_users.emp_sap_id as emp_sap_id,
...
def latestQuantitativeIncentiveUsers(request):
    ...
    if latest_scheme :
        ...
        users = SpUserSchemes.objects.raw(''' SELECT sp_user_schemes.*, sp_users.id as user_id,sp_users.first_name as first_name,sp_users.middle_name as middle_name,sp_users.last_name as last_name,sp_users.store_name as store_name,sp_users.emp_sap_id as emp_sap_id,
        sp_user_area_allocations.town_name,sp_user_area_allocations.route_name,sp_user_area_allocations.state_name FROM sp_user_schemes
        LEFT JOIN sp_users on sp_users.id = sp_user_schemes.user_id
        LEFT JOIN sp_user_area_allocations on sp_user_area_allocations.user_id = sp_user_schemes.user_id
...
def editFreeScheme(request,scheme_id):
    if request.method == "POST":
        ...
        context['users'] = SpUserSchemes.objects.raw(''' SELECT sp_user_schemes.*,sp_users.id as user_id,sp_users.first_name as first_name,sp_users.middle_name as middle_name,sp_users.last_name as last_name,sp_users.store_name as store_name,sp_users.emp_sap_id as emp_sap_id,
        sp_user_area_allocations.town_name,sp_user_area_allocations.route_name,sp_user_area_allocations.state_name FROM sp_user_schemes
        LEFT JOIN sp_users on sp_users.id = sp_user_schemes.user_id
        LEFT JOIN sp_user_area_allocations on sp_user_area_allocations.user_id = sp_user_schemes.user_id
        WHERE sp_user_schemes.scheme_id = %s
...
def editFlatIncentiveScheme(request,scheme_id):
    if request.method == "POST":
        ...
        context['users'] = SpUserFlatSchemes.objects.raw(''' SELECT sp_user_flat_schemes.*, sp_users.id as user_id,sp_users.first_name as first_name,sp_users.middle_name as middle_name,sp_users.last_name as last_name,sp_users.store_name as store_name,sp_users.emp_sap_id as emp_sap_id,
        sp_user_area_allocations.town_name,sp_user_area_allocations.route_name,sp_user_area_allocations.state_name FROM sp_user_flat_schemes
        LEFT JOIN sp_users on sp_users.id = sp_user_flat_schemes.user_id
        LEFT JOIN sp_user_area_allocations on sp_user_area_allocations.user_id = sp_user_flat_schemes.user_id
        WHERE sp_user_flat_schemes.scheme_id = %s
            ...
            if applied_on_variant_id != '' :
                if route_ids != "" and is_route == '0':
                    ...
                    for route_id in route_ids :
                        ...
                        users = SpUserAreaAllocations.objects.raw('''SELECT sp_user_area_allocations.*, sp_users.id as user_id,sp_users.first_name as first_name,sp_users.middle_name as middle_name,sp_users.last_name as last_name,sp_users.store_name as store_name,sp_users.emp_sap_id as emp_sap_id
...
@login_required
def importOrderList(request):
    today      = date.today()
    today_date = today.strftime("%d/%m/%Y")
    today = datetime.strptime(str(today_date), '%d/%m/%Y').strftime('%Y-%m-%d')
    if request.method == 'POST' :
        order_sheet_template = request.FILES["order_sheet"]
        wb = openpyxl.load_workbook(order_sheet_template)
        worksheet = wb.active
        excel_data = list()
        header_data = list()
        product_name = list()
        ...
        item_code           = []
        item_code_errors    = []
        for id, header in enumerate(header_data[0]):
            header_list.append(header)
            item_code_error     = ''
            if id>2:
                if SpProductVariants.objects.filter(item_sku_code=header).exists():
                    item_code_error     = ''
                else:
                    item_code_error     = "Invalid Item Code"
                    error_list.append('0')
                item_code.append(header)
            if item_code_error:
                error_list.append('1')     
            item_code_errors.append(item_code_error)
            
        for id, header in enumerate(product_name[0]):
            product_name_list.append(header)  
            
        
        distributor_data = []
        for id, data in enumerate(excel_data):
            temp = {}
            temp['vehicle_no']              = data[0]
            if SpVehicles.objects.filter(registration_number=data[0]).exists():
                temp['vehicle_no_error']     = ''
            else:
                temp['vehicle_no_error']     = "Invalid Vehicle No"
                error_list.append('0')
            
            temp['sap_id']              = data[1]
            if SpUsers.objects.filter(emp_sap_id=data[1], is_distributor=1).exists():
                temp['sap_id_error']     = ''
            else:
                temp['sap_id_error']     = "Invalid SAP ID"
                error_list.append('0')
            try:
                users = SpUsers.objects.get(emp_sap_id=data[1])
                if  SpOrders.objects.filter(user_id=users.id, order_date__icontains=today).exists():
                    temp['order_id_error']     = "Order already placed of this user."
                    error_list.append('1')
                temp['distributor_name']     = data[2]
            except SpUsers.DoesNotExist:
                temp['sap_id_error']     = "Invalid SAP ID"
                error_list.append('0')
           
            product_list = []
            for x in range(3, int(len(data))):
                
                if data[x]!="None":
                    product_list.append(data[x])
                else:
                    product_list.append(0)
            temp['product_list']        = product_list
            distributor_data.append(temp)
        
        error_length = int(len(error_list))
        item_code_values = ','.join(str(e) for e in item_code)
        
        context = {}
        context['excel_data']           = distributor_data
        context['header_list']          = zip(header_list,product_name_list)
        context['item_code']            = item_code
        context['item_code_values']     = item_code_values
        context['item_code_errors']     = item_code_errors
        context['error_length']         = error_length
        context['today_dates']          =  today
        context['today_date']           =  today_date
        context['page_title']           = "Upload Orders"
        template = 'schemes/upload-order-list.html'
        return render(request,template,context)
...
@login_required
def updateOrderList(request):
    if request.method == 'POST' :
        today           = request.POST['today_date']
        order_date      = datetime.strptime(str(today),'%Y-%m-%d').strftime('%Y-%m-%d %H:%M:%S')
        sap_id          = request.POST.getlist('sap_id[]')
        all_vehicle_no  = request.POST.getlist('vehicle_no[]')
        sap_id          = set(sap_id)
        sap_id          = list(sap_id)
        all_vehicle_no  = set(all_vehicle_no)
        all_vehicle_no  = list(all_vehicle_no)
        item_sku_code   = request.POST['item_code']
        item_sku_code   = item_sku_code.split(',')
        user_list       = []
        new_sap_id      = []
        new_item_code   = []
        for id, val in enumerate(sap_id):
            if SpUsers.objects.filter(emp_sap_id=sap_id[id]).exists():
                if not SpOrders.objects.filter(user_sap_id=sap_id[id], order_date__icontains=today).exists():
                    users = SpUsers.objects.get(emp_sap_id=sap_id[id])
                    user  = SpBasicDetails.objects.get(user_id=users.id)        
                    ...
                    user_area_details = SpUserAreaAllocations.objects.get(user_id=users.id)
                    ...
                    order.user_sap_id           =  getModelColumnById(SpUsers, users.id, 'emp_sap_id')
                    order.user_name             =  getUserName(users.id)
                    order.user_type             =  getUserRole(users.id)
                    ...
                    order.amount_to_be_paid     =  0
                    order.outstanding_amount    =  user.outstanding_amount
                    production_unit_id          =  getModelColumnByColumnId(SpBasicDetails, 'user_id', users.id, 'production_unit_id')
                    ...
                    # if getModelColumnByColumnId(SpBasicDetails, 'user_id', users.id, 'tcs_applicable') == 1:
                    #     order.tcs_value      =  getModelColumnByColumnId(SpBasicDetails, 'user_id', users.id, 'tcs_value')
                    order.save()  
    
                    total_order_amount          = []
                    total_order_quantity        = []
                    for ids, val in enumerate(item_sku_code):
                        key = 'qty_'+sap_id[id]+''+val+''+'[]'
                        sumvalue=request.POST.getlist(key)
                        sumvalue=sum([float(num) for num in sumvalue])
                        if sumvalue > 0:
                            quantity                 = float(sumvalue)
                            product_details          = SpProductVariants.objects.get(item_sku_code=val)  
                            pouch_quantity           = int(float(quantity)*float(product_details.no_of_pouch))
                            quantity_in_ltr          = float(pouch_quantity)*float(product_details.variant_size)
                            user_product_details     = SpUserProductVariants.objects.get(user_id=users.id, item_sku_code=val)  
                            rate                     = round(float(user_product_details.container_sp_user),2)
                            amount                   = round((float(quantity)*float(rate)),2)
                            if product_details.gst:
                                taxable_amount = round((float(amount)*int(product_details.gst)),2)
                            else:
                                taxable_amount = 0 
                            final_amount = round((float(amount)+float(taxable_amount)),2)  
                            total_order_amount.append(amount)
                            total_order_quantity.append(quantity)
                            item                            = SpOrderDetails()
                            item.user_id                    = users.id
                            item.order_id                   = order.id
                            item.product_id                 = product_details.product_id 
                            item.product_name               = product_details.product_name
                            item.product_variant_id         = product_details.id
                            item.product_variant_name       = product_details.variant_name	
                            item.product_variant_size       = product_details.variant_size
                            item.product_no_of_pouch        = product_details.no_of_pouch
                            item.product_container_size     = product_details.container_size
                            item.product_container_type     = product_details.container_name
                            item.quantity_in_pouch          = pouch_quantity
                            item.quantity_in_ltr            = round(quantity_in_ltr,2)
                            item.quantity                   = quantity
                            item.rate                       = rate
                            item.amount                     = amount
                            item.packaging_type             = 0
                            if product_details.gst:
                                item.gst                    = product_details.gst
                            item.taxable_amount             = taxable_amount
                            item.final_amount               = final_amount
                            item.product_packaging_type_name= product_details.packaging_type_name
                            item.is_allow                   = product_details.is_allow_in_packaging
                            item.tally_export_type          = 0
                            item.order_date                 = order_date
                            item.save()
                            
                            free_scheme = None
                            scheme_details              = SpUserSchemes.objects.filter(applied_on_variant_id=product_details.id, scheme_type=1, user_id=users.id, status=1).values('id', 'scheme_start_date', 'scheme_end_date') 
                            for free in scheme_details:
                                if free['scheme_start_date'] and free['scheme_end_date']:
                                    try:
                                        free_scheme = SpUserSchemes.objects.get(id=free['id'], scheme_start_date__lte=today, scheme_end_date__gte=today)
                                    except SpUserSchemes.DoesNotExist:
                                        free_scheme = None
                                elif free['scheme_start_date'] and free['scheme_end_date'] is None:
                                    try:
                                        free_scheme = SpUserSchemes.objects.get(id=free['id'], scheme_start_date__lte=today)
                                    except SpUserSchemes.DoesNotExist:
                                        free_scheme = None                                    
                            ...
                            if free_scheme:
                                ...
                                if free_scheme.pouch_quantity>0:
                                    if free_scheme.packaging_type == 0:
                                        order_value = float(quantity)/float(free_scheme.minimum_order_quantity)
                                        ...
                                        pouch_value = int(float(quantity)*float(getModelColumnById(SpProductVariants, free_scheme.applied_on_variant_id, 'no_of_pouch')))
                                        if int(pouch_value)>0:
                            ...
                            flat_scheme = None
                            scheme_details              = SpUserFlatSchemes.objects.filter(applied_on_variant_id=product_details.id,user_id=users.id,incentive_amount__gt=0, status=1).values('id', 'scheme_start_date', 'scheme_end_date') 
                            for flatv in scheme_details:
                                if flatv['scheme_start_date'] and flatv['scheme_end_date']:
                                    try:
                                        flat_scheme = SpUserFlatSchemes.objects.get(id=flatv['id'], scheme_start_date__lte=today, scheme_end_date__gte=today)
                                    except SpUserFlatSchemes.DoesNotExist:
                                        flat_scheme = None
                                elif flatv['scheme_start_date'] and flatv['scheme_end_date'] is None:
                                    try:
                                        flat_scheme = SpUserFlatSchemes.objects.get(id=flatv['id'], scheme_start_date__lte=today)
                                    except SpUserFlatSchemes.DoesNotExist:
                                        flat_scheme = None
                            if flat_scheme:           
                                if quantity> 0:
                                    pouch_quantity      = int(float(quantity)*float(getModelColumnById(SpProductVariants, flat_scheme.applied_on_variant_id, 'no_of_pouch')))
                                    flat_discount       = (float(quantity)*int(getModelColumnById(SpProductVariants, flat_scheme.applied_on_variant_id, 'no_of_pouch')))*float(getModelColumnById(SpProductVariants, flat_scheme.applied_on_variant_id, 'variant_size'))
                    ...
                    toddy = today  = date.today()
                    newtoday                 = toddy.strftime("%Y-%m-%d")
                    current                  = int((toddy.strftime("%Y")))
                    prev                     = int(toddy.strftime("%Y"))-1
                    naxt                     = int(toddy.strftime("%Y"))+1
                    ...
                    order_amounts = SpOrders.objects.filter(user_id=users.id,block_unblock=1,order_date__range=[financial_year_start_date,newtoday]).aggregate(Sum('order_total_amount'))['order_total_amount__sum']  
                    if order_amounts:
                    ...
                    order.order_items_count     =  len(total_order_quantity)
                    order.save()
                    for ids, val in enumerate(item_sku_code):
                        for vds, vals in enumerate(all_vehicle_no):
                            key = 'veh_'+sap_id[id]+''+vals+''+val+''+'[]'
                            vvalue=request.POST.getlist(key)
                            vvalue=sum([float(num) for num in vvalue])   
                            if vvalue > 0: 
                                quantity                 = float(vvalue)
                                product_details          = SpProductVariants.objects.get(item_sku_code=val)  
                                pouch_quantity           = int(float(quantity)*int(product_details.no_of_pouch))
                                quantity_in_ltr          = float(pouch_quantity)*float(product_details.variant_size)
                                user_product_details     = SpUserProductVariants.objects.get(user_id=users.id, item_sku_code=val)  
                                rate                     = round(float(user_product_details.container_sp_user),2)
                                amount                   = round((float(quantity)*float(rate)),2)
                                ...
                                total_order_amount.append(amount)
                                total_order_quantity.append(float(quantity)) 
                                vehicle                            = SpLogisticPlanDetail()    
                                vehicle.user_id                    = users.id
                                vehicle.order_id                   = order.id
                                vehicle.product_id                 = product_details.product_id 
                                vehicle.product_name               = product_details.product_name 
                                vehicle.product_variant_id         = product_details.id
                                vehicle.product_variant_name       = product_details.variant_name 
                                vehicle.product_variant_size       = product_details.variant_size
                                vehicle.product_no_of_pouch        = product_details.no_of_pouch
                                vehicle.product_container_size     = product_details.container_size
                                vehicle.product_container_type     = product_details.container_name
                                vehicle.quantity_in_pouch          = pouch_quantity
                                vehicle.quantity_in_ltr            = round(quantity_in_ltr,2)
                                vehicle.quantity                   = quantity
                                vehicle.rate                       = rate
                                vehicle.amount                     = amount
                                vehicle.packaging_type             = 0
                                if product_details.gst:
                                    vehicle.gst                   = product_details.gst
                                vehicle.product_packaging_type_name= product_details.packaging_type_name
                                vehicle.is_allow                    = product_details.is_allow_in_packaging
                                vehicle.order_date                      = order_date    
                                route_id                                = getModelColumnByColumnId(SpVehicles,'registration_number',vals,'route_id') 
                                if route_id:  
                                    vehicle.route_id      =route_id
                                route_name                 = getModelColumnByColumnId(SpVehicles,'registration_number',vals,'route_name')
                                if route_name:  
                                    vehicle.route_name      =route_name      
                                vehicle_id                              = getModelColumnByColumnId(SpVehicles,'registration_number',vals,'id') 
                                if vehicle_id:  
                                    vehicle.vehicle_id                  =vehicle_id   
                                vehicle.vehicle_number                      = vals 
                                vehicle.save()
                                ...
                                if free_scheme:
                                    ...
                                    if free_scheme.pouch_quantity > 0:
                                        if free_scheme.packaging_type == 0:
                                            ...
                                            pouch_value = int(float(quantity)*int(getModelColumnById(SpProductVariants, free_scheme.applied_on_variant_id, 'no_of_pouch')))
                                            if int(pouch_value)>0:
                                                order_value = int(pouch_value)/int(free_scheme.minimum_order_quantity)
                                                order_value = str(order_value).split('.')
                                                order_value = int(order_value[0])
                                    ...
                                    if int(pouch_quantity) > 0:
                                        ...
                                        logistic_order_scheme.user_id                     = users.id
                                        ...
                                        route_id                                          = getModelColumnByColumnId(SpVehicles,'registration_number',vals,'route_id') 
                                        if route_id:  
                                            logistic_order_scheme.route_id                =route_id
                                        route_name                                        = getModelColumnByColumnId(SpVehicles,'registration_number',vals,'route_name')
                                        if route_name:  
                                            logistic_order_scheme.route_name              =route_name      
                                        vehicle_id                                        = getModelColumnByColumnId(SpVehicles,'registration_number',vals,'id') 
                                        if vehicle_id:  
                                            logistic_order_scheme.vehicle_id              =vehicle_id   
                                        logistic_order_scheme.vehicle_number              = vals 
                    ...
                    response = {}
                    response['flag'] = False
                    response['message'] = "order allredy placed."
                    return JsonResponse(response)
        #Save Activity
        user_name   = request.user.first_name+' '+request.user.middle_name+' '+request.user.last_name
        heading     = 'Order List updated'
        activity    = 'Order List updated by '+user_name+' on '+datetime.now().strftime('%d/%m/%Y | %I:%M %p') 
        saveActivity('Order Management', 'Order List', heading, activity, request.user.id, user_name, 'schemeEdit.png', '1', 'web.png')
...
def exportOrderExcel(request):
    path = 'excel-templates/order_sheet_template.xlsx'
    if os.path.exists(path):
        with open(path, 'rb') as fh:
            response = HttpResponse(fh.read(
            ), content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            response['Content-Disposition'] = 'attachment; filename=order_sheet_template.xlsx'

            #Save Activity
            user_name = request.user.first_name+' ' + \
                request.user.middle_name+' '+request.user.last_name
            heading = 'Order excel format exported'
            activity = 'Order excel format exported by '+user_name + \
                ' on '+datetime.now().strftime('%d/%m/%Y | %I:%M %p')
            saveActivity('Order Management', 'Export Order', heading, activity,
                         request.user.id, user_name, 'generateIndentReport.png', '1', 'web.png')
