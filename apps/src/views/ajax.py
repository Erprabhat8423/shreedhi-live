...
# get user recive crate list
@login_required
def getUserReciveCrateDetail(request):
    crate_date                   = request.GET['crate_date']
    user_id                      = request.GET['user_id']
    response = {}
    msg=""
    # try:
    #     user_crate= SpUserCrateLedger.objects.get(user_id=user_id, normal_credit__gt=0, updated_datetime__icontains=crate_date)
    # except SpUserCrateLedger.DoesNotExist:
    #     user_crate=None
    try:
        user_crate= SpUserCrateLedger.objects.filter(user_id=user_id, updated_datetime__icontains=crate_date).aggregate(Sum('normal_credit'))['normal_credit__sum']
    except SpUserCrateLedger.DoesNotExist:
        user_crate=None
    if user_crate > 0:
        # if  user_crate.normal_credit and user_crate.jumbo_credit==0: 
        msg="already recive "+ str(user_crate) +" normal crate." 
        # elif  user_crate.jumbo_credit and user_crate.normal_credit==0: 
        #     msg="already recive " + str(user_crate.jumbo_debit) +" jumbo crate."    
        # else:
        #     msg="already recive "+ str(user_crate.normal_credit) +" normal crate and " + str(user_crate.jumbo_credit) +" jumbo crate."    
    response['msg'] = msg
    return JsonResponse(response)
...
def getUserCrateDetail(request):
    ...
    msg=""
    try:
        user_crate= SpUserCrateLedger.objects.filter(user_id=user_id, updated_datetime__icontains=crate_date).aggregate(Sum('normal_debit'))['normal_debit__sum']
    except SpUserCrateLedger.DoesNotExist:
        user_crate=None
    if user_crate > 0:
        # if  user_crate.normal_debit and user_crate.jumbo_debit==0: 
        msg="already dispatch "+ str(user_crate) +" normal crate." 
...
def productOption(request):
    ...
    for variant in variants :
        options += "<option value="+str(variant.product_variant_id)+" data-rc="+str(variant.product_limit)+">"+variant.variant_name+"</option>"
...
def getproductOption(request):
    response = {}
    options = '<option value="" selected>Select Product Variant</option>'
    variants = SpUserProductVariants.objects.filter(product_id=request.GET['product_id'], user_id=request.GET['user_id'],status=1)
    for variant in variants :
        options += "<option value="+str(variant.product_variant_id)+" data-rc="+str(variant.product_limit)+">"+variant.variant_name+"</option>"
...
def getSchemeDetails(request):
    ...
    bonus_scheme_text = ''
    try:
        free_scheme = SpUserSchemes.objects.get(applied_on_variant_id=request.GET['product_variant_id'], user_id=request.GET['user_id'], scheme_start_date__lte=today.strftime("%Y-%m-%d"), scheme_end_date__gte=today.strftime("%Y-%m-%d"), scheme_type=1, status=1)
    except SpUserSchemes.DoesNotExist:
        free_scheme = None
    
    if free_scheme and request.GET['quantity']:
        if float(request.GET['quantity']) >= int(free_scheme.minimum_order_quantity):
            free = ''
            if free_scheme.pouch_quantity>0:
                if request.GET['packaging_type'] == '0':
                    ...
                    quantity = float(free_scheme.pouch_quantity) * \
                        float(request.GET['quantity'])
                    if quantity == 1:
                        free += ' '+str(int(quantity))+' free '+str(free_scheme.order_packaging_name)+'.'
                    else:    
                        free += ' '+str(int(quantity))+' free '+str(free_scheme.order_packaging_name)+'.'
                else:    
                    quantity = float(request.GET['quantity'])/int(getModelColumnById(
                        SpProductVariants, free_scheme.free_variant_id, 'no_of_pouch'))
                    quantity = str(quantity).split('.')
                    quantity = float(quantity[0])
                    if quantity > 0:
                        quantity = float(quantity) * \
                            int(free_scheme.pouch_quantity)
                        free += 'Free '+getModelColumnById(SpProductVariants, free_scheme.free_variant_id, 'variant_name')+'-'
                        if quantity == 1:
                            free += ' '+str(int(quantity))+' free ' + \
                                str(free_scheme.order_packaging_name)+'.'
                        else:    
                            free += ' '+str(int(quantity))+' free ' + \
                                str(free_scheme.order_packaging_name)+'.'
                    else:
    ...
    try:
        flat_scheme = SpUserFlatSchemes.objects.get(applied_on_variant_id=request.GET['product_variant_id'], user_id=request.GET['user_id'], scheme_start_date__lte=today.strftime("%Y-%m-%d"), scheme_end_date__gte=today.strftime("%Y-%m-%d"), incentive_amount__gt=0, status=1)
    except SpUserFlatSchemes.DoesNotExist:
        flat_scheme = None
    
    if flat_scheme and request.GET['quantity']:
        if float(request.GET['quantity']) > 0:
            if request.GET['packaging_type'] == '0':
...
def getBonusSchemeDetails(request):
    ...
    if len(variants_quantity) > 0:
        variant_quantity = sum(float(x) for x in variants_quantity)
    else:
        variant_quantity = 0
    ...
    return JsonResponse(response)
#--------------------for ledger user------------------
def getOrganizationTousers(request):
    organization_id = request.GET['organization_id']
    response = {}
    if organization_id:
        users = SpUsers.objects.filter(organization_id=organization_id, status=1, is_distributor=1)
    else:
    ...
    if users:
        for user in users:
            # store_name = getModelColumnById(SpUsers, user.id, 'store_name')
            options += "<option value="+str(user.id) + ">"+str(user.store_name) + "("+str(user.emp_sap_id)+")</option>"
    response['options'] = options
    return JsonResponse(response)
...
def getBank(request):
    organization_id = request.GET['organization_id']
    response = {}
    banks = SpBankDetails.objects.filter(organization_id__icontains=organization_id, status=1)
    
    options = '<option value="">Select Bank</option>'
    if banks:
        for bank in banks:
            # store_name = getModelColumnById(SpUsers, user.id, 'store_name')
            options += "<option value=" + str(bank.id) + ">"+str(bank.bank_name) + "("+str(bank.account_no)+")</option>"
    response['options'] = options
    return JsonResponse(response)
