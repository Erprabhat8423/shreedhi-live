...
def productVariantLists(request):
    ...
    for product_class in product_classes:
        ...
        for product in product_class['products']:
            ...
            for product_variant in product_variants:
                ...
                variants['product_limit']          = product_variant.product_limit 
    ...
    # except SpBasicDetails.DoesNotExist:
    #     outstanding_amount = None
    last_balance                = SpUserLedger.objects.filter(user_id=request.data.get("user_id")).values('balance').last()
    if last_balance:
        outstanding_amount = last_balance['balance']
    else:
        outstanding_amount = None 
...
def saveOrder(request):
    ...
    order_amounts = SpOrders.objects.filter(user_id=request.data.get("user_id"),block_unblock=1,order_date__range=[financial_year_start_date,newtoday]).aggregate(Sum('order_total_amount'))['order_total_amount__sum'] 
    if order_amounts:
        order_amounts = round((order_amounts+float(order_details['total_order_amount'])),2)
    else:
        order_amounts = order_details['total_order_amount']
    ...
    for order_item in order_item_list:
        # if not SpOrderDetails.objects.filter(user_id = request.data.get("user_id"),product_variant_id = order_item['product_variant_id'],order_date__icontains = datetime.now().strftime('%Y-%m-%d')).exists():
            if order_item['packaging_type'] == '0':
                pouch_quantity     = int(float(order_item['qty'])*float(getModelColumnById(SpProductVariants, order_item['product_variant_id'], 'no_of_pouch')))
                ...
                pouch_quantity     = int(float(order_item['qty']))
            ...
            vehicle.user_id                    = request.data.get("user_id")
            vehicle.order_id                   = order.id
            vehicle.product_id                 = order_item['product_id']
            vehicle.product_name               = order_item['product_name']
            vehicle.product_variant_id         = order_item['product_variant_id']
            vehicle.product_variant_name       = order_item['product_variant_name'] 
            vehicle.product_variant_size       = order_item['product_variant_size']
            vehicle.product_no_of_pouch        = getModelColumnById(SpProductVariants, order_item['product_variant_id'], 'no_of_pouch')
            vehicle.product_container_size     = order_item['product_container_size']
            vehicle.product_container_type     = getModelColumnById(SpProductVariants, order_item['product_variant_id'], 'container_name')
            vehicle.quantity_in_pouch          = pouch_quantity
            vehicle.quantity_in_ltr            = round(quantity_in_ltr,2)
            vehicle.quantity                   = order_item['qty']
            vehicle.rate                       = order_item['rate']
            vehicle.amount                     = order_item['amount']
            vehicle.packaging_type             = order_item['packaging_type']
            vehicle.gst                        = getModelColumnById(SpProductVariants, order_item['product_variant_id'], 'gst')
            vehicle.product_packaging_type_name= getModelColumnById(SpProductVariants, order_item['product_variant_id'], 'packaging_type_name')
            vehicle.is_allow                   = getModelColumnById(SpProductVariants, order_item['product_variant_id'], 'is_allow_in_packaging')
            vehicle.order_date                 = datetime.now().strftime('%Y-%m-%d %H:%M:%S')    
            vehicle.route_id                   = getModelColumnByColumnId(SpUserAreaAllocations,'user_id',request.data.get("user_id"), 'route_id')    
            vehicle.route_name                 = getModelColumnByColumnId(SpUserAreaAllocations,'user_id',request.data.get("user_id"), 'route_name')      
            vehicle.vehicle_id                 = getModelColumnByColumnId(SpBasicDetails,'user_id',request.data.get("user_id"), 'vehicle_id') 
            vehicle.vehicle_number             = vehicle_no
            vehicle.save()

    user_name   = request.user.first_name+' '+request.user.middle_name+' '+request.user.last_name
    heading     = 'New Order('+order.order_code+') has been initiated'+str(order_item_list)
    ...
    if len(free_scheme_list):
        for free_scheme in free_scheme_list:
            if int(float(free_scheme['on_order_of']))>0:
                ...
                order_scheme.on_order_of        = int(float(free_scheme['on_order_of']))
                ...
                logistic_order_scheme.user_id                     = request.data.get("user_id")
                logistic_order_scheme.scheme_id                   = free_scheme['scheme_id']
                logistic_order_scheme.scheme_type                 = "free"
                logistic_order_scheme.variant_id                  = free_scheme['variant_id']
                logistic_order_scheme.on_order_of                 = int(float(free_scheme['on_order_of']))
                logistic_order_scheme.free_variant_id             = free_scheme['free_variant_id']
                if 'free_variant_container_type' in free_scheme:
                    logistic_order_scheme.free_variant_container_type = free_scheme['free_variant_container_type']
                logistic_order_scheme.container_quantity = free_scheme['free_container_quantity']
                ...
                logistic_order_scheme.pouch_quantity              = free_scheme['free_pouch_quantity']
                logistic_order_scheme.quantity_in_ltr             = int(free_scheme['free_pouch_quantity'])*float(getModelColumnById(SpProductVariants,free_scheme['free_variant_id'],'variant_size'))
                ...
                logistic_order_scheme.route_id                    = getModelColumnByColumnId(SpUserAreaAllocations,'user_id',request.data.get("user_id"), 'route_id')  
                logistic_order_scheme.route_name                  = getModelColumnByColumnId(SpUserAreaAllocations,'user_id',request.data.get("user_id"), 'route_name')
                logistic_order_scheme.vehicle_id                  = getModelColumnByColumnId(SpBasicDetails,'user_id',request.data.get("user_id"), 'vehicle_id') 
                logistic_order_scheme.vehicle_number              = vehicle_no
                logistic_order_scheme.created_at                  = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    if len(quantitative_scheme):
        ...
        logistic_order_scheme.order_id               = order.id
        logistic_order_scheme.user_id                = request.data.get("user_id")
        logistic_order_scheme.scheme_id              = quantitative_scheme['scheme_id']
        logistic_order_scheme.scheme_type            = "quantitative"
        logistic_order_scheme.free_variant_id = quantitative_scheme['free_variant_id']
        if 'free_variant_container_type' in quantitative_scheme:
            logistic_order_scheme.free_variant_container_type = quantitative_scheme['free_variant_container_type']
        logistic_order_scheme.container_quantity     = quantitative_scheme['free_container_quantity']
        logistic_order_scheme.pouch_quantity         = quantitative_scheme['free_pouch_quantity']
        logistic_order_scheme.quantity_in_ltr        = int(quantitative_scheme['free_pouch_quantity'])*float(getModelColumnById(SpProductVariants,quantitative_scheme['free_variant_id'],'variant_size'))
        logistic_order_scheme.product_id             = getModelColumnById(SpProductVariants,quantitative_scheme['free_variant_id'],'product_id')
        logistic_order_scheme.product_class_id       = getModelColumnById(SpProductVariants,quantitative_scheme['free_variant_id'],'product_class_id')
        logistic_order_scheme.route_id               = getModelColumnByColumnId(SpUserAreaAllocations,'user_id',request.data.get("user_id"), 'route_id')  
        logistic_order_scheme.route_name             = getModelColumnByColumnId(SpUserAreaAllocations,'user_id',request.data.get("user_id"), 'route_name')
        logistic_order_scheme.vehicle_id             = getModelColumnByColumnId(SpBasicDetails,'user_id',request.data.get("user_id"), 'vehicle_id') 
        logistic_order_scheme.vehicle_number         = vehicle_no
    ...
    if len(flat_schemes):
        for flat_scheme in flat_schemes:
            ...
            logistic_order_scheme.order_id           = order.id
            logistic_order_scheme.user_id            = request.data.get("user_id")
            logistic_order_scheme.scheme_id          = int(flat_scheme['scheme_id'])
            logistic_order_scheme.scheme_type        = "flat"
            logistic_order_scheme.incentive_amount   = flat_scheme['incentive_amount']
            logistic_order_scheme.variant_id         = flat_scheme['applied_on_variant_id']
            logistic_order_scheme.quantity_in_ltr    = flat_scheme['quantity_in_ltr']
    ...
    if len(bulk_scheme):
        ...
        logistic_order_scheme.order_id               = order.id
        logistic_order_scheme.user_id                = request.data.get("user_id")
        logistic_order_scheme.scheme_id              = bulk_scheme['scheme_id']
        logistic_order_scheme.scheme_type            = "bulkpack"
        logistic_order_scheme.incentive_amount       = bulk_scheme['incentive_amount']
        logistic_order_scheme.unit_id                = bulk_scheme['unit_id']
        logistic_order_scheme.unit_name              = bulk_scheme['unit_name']
        logistic_order_scheme.route_id               = getModelColumnByColumnId(SpUserAreaAllocations,'user_id',request.data.get("user_id"), 'route_id')  
        logistic_order_scheme.route_name             = getModelColumnByColumnId(SpUserAreaAllocations,'user_id',request.data.get("user_id"), 'route_name')
        logistic_order_scheme.vehicle_id             = getModelColumnByColumnId(SpBasicDetails,'user_id',request.data.get("user_id"), 'vehicle_id') 
        logistic_order_scheme.vehicle_number         = vehicle_no
...
def updateOrder(request):
    ...
    if request.data.get("amount_to_be_paid") is None or request.data.get("amount_to_be_paid") == '':
        return Response({'message': 'Amount field is required', 'response_code': HTTP_400_BAD_REQUEST}, status=HTTP_400_BAD_REQUEST) 
    if SpOrders.objects.filter(id=request.data.get("order_id"),order_status__gt = 1).exists():
        return Response({'message': 'Order is Forwarded Contact to Admin!', 'response_code': HTTP_400_BAD_REQUEST}, status=HTTP_400_BAD_REQUEST) 
    ...
    order.production_unit_id    =  production_unit_id
    order.production_unit_name  =  getModelColumnById(SpProductionUnit, production_unit_id, 'production_unit_name')
    order.platform_updated      =  'APP'
    # if newtoday == datetime.now().strftime('%Y-%m-%d'):
        # today_date                                  = datetime.now().strftime('%Y')+'-'+'04'+'-'+'01'
        # if newtoday > today_date:
        #     financial_year_start_date               = datetime.now().strftime('%Y')+'-'+'04'+'-'+'01'
        # else:
        ...
        # order_amounts                               = SpOrders.objects.filter(user_id=request.data.get("user_id"),block_unblock=1,order_date__range=[financial_year_start_date,newtoday]).aggregate(Sum('order_total_amount'))['order_total_amount__sum']  
        # if order_amounts:
        #     order_amounts = round((order_amounts+order_details['total_order_amount']),2)
        # else:
        #   order_amounts = order_details['total_order_amount']
        # order_amounts                               = round((order_amounts+float()),2)
        # tcsvalues                                   = SpTcsMaster.objects.filter().first()
        # if int(order_amounts) > tcsvalues.tcs_value:
        #     order.tcs_value                         =  tcsvalues.tcs_percentage
        # else:
        #     order.tcs_value                         = None
    ...
    #SpLogisticPlanDetail.objects.filter(order_id=request.data.get("order_id")).delete()
    #save order item details
    for order_item in order_item_list:
        if order_item['packaging_type'] == '0':
            pouch_quantity     = int(float(order_item['qty'])*float(getModelColumnById(SpProductVariants, order_item['product_variant_id'], 'no_of_pouch')))
        ...
        if SpLogisticPlanDetail.objects.filter(order_id=item.order_id, product_variant_id=item.product_variant_id).exists():
            vehicle                            = SpLogisticPlanDetail.objects.get(order_id=item.order_id, product_variant_id=item.product_variant_id)    
            vehicle.user_id                    = request.data.get("user_id")
            vehicle.order_id                   = order.id
            vehicle.product_id                 = order_item['product_id']
            vehicle.product_name               = order_item['product_name']
    ...
    SpLogisticOrderSchemes.objects.filter(order_id=request.data.get("order_id")).delete()
    if len(free_scheme_list):
        for free_scheme in free_scheme_list:
            if int(float(free_scheme['on_order_of']))>0:
                order_scheme                    = SpOrderSchemes()
    ...
    user_name   = request.user.first_name+' '+request.user.middle_name+' '+request.user.last_name
    heading     = 'Order('+order.order_code+') has been updated'+str(order_item_list)
...
def orderDetails(request):
    ...
    if request.data.get("order_id") is None or request.data.get("order_id") == '':
        ...
        if order_details:
            ...
            change_date                 = datetime.strptime(str(order_date), '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d')
            if change_date == today.strftime("%Y-%m-%d"):
                last_balance            = SpUserLedger.objects.filter(user_id=request.data.get("user_id")).values('balance').last()
                if last_balance:
                    order_details.outstanding_amount = round((float(last_balance['balance'])+float(order_details.order_total_amount)),2)
                else:
                    order_details.outstanding_amount = order_details.order_total_amount
            else:
                last_balance            = SpUserLedger.objects.filter(user_id=request.data.get("user_id"),order_date__icontains =change_date ).values('balance').last()
                if last_balance:
                    order_details.outstanding_amount = last_balance['balance']
                else:
                    order_details.outstanding_amount = None 
    ...
    try:
        ...
        last_balance            = SpUserLedger.objects.filter(user_id=request.data.get("user_id")).values('balance').last()
        last_crate_balance      = SpUserCrateLedger.objects.filter(user_id=request.data.get("user_id")).values('normal_balance').last()
        if last_balance:
            outstanding_amount  = last_balance['balance']
        else:
            uoutstanding_amount = 0
        if last_crate_balance:
            opening_crates      = last_crate_balance['normal_balance']
        else:
            opening_crates      = 0
    ...
    if request.data.get("type") == '0':
        ...
        for order_item in order_list:
            ...
            if change_date == today.strftime("%Y-%m-%d"):
                last_balance            = SpUserLedger.objects.filter(user_id=request.data.get("user_id")).values('balance').last()
                if last_balance:
                    order_item['outstanding_amount'] = round((float(last_balance['balance'])+float(order_item['order_total_amount'])),2)
                else:
                    order_item['outstanding_amount'] = order_item['order_total_amount']
            else:
                last_balance            = SpUserLedger.objects.filter(user_id=request.data.get("user_id"),order_date__icontains =change_date ).values('balance').last()
                if last_balance:
                    order_item['outstanding_amount'] = last_balance['balance']
                else:
                    order_item['outstanding_amount'] = None 
            order_item['order_date']  = datetime.strptime(str(order_date), '%Y-%m-%d %H:%M:%S').strftime('%d/%m/%Y | %I:%M:%p')
        if order_list:
