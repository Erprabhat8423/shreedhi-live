...
def getUserOrderList(request):
    ...
    try:
        driver_details = SpVehicles.objects.get(route_id=request.data.get("route_id"))
    ...
    today   = date.today()
    user_list = SpOrders.objects.filter(order_date__icontains=today.strftime("%Y-%m-%d"), route_id=request.data.get("route_id")).values('id', 'user_id', 'user_name', 'route_id', 'route_name', 'order_date', 'order_status')
    for order_details in user_list:
        if SpPlantCrateLedger.objects.filter(updated_datetime__icontains=today.strftime("%Y-%m-%d"), plant_user_id=request.user.id, user_id=order_details['user_id']).exists():
        ...
        order_details['product_list']   = SpOrderDetails.objects.filter(order_id=order_details['id']).values('id', 'order_id', 'product_id', 'product_name', 'product_variant_id', 'product_variant_name', 'quantity', 'rate', 'amount', 'order_date')
    ...
    context['driver_contact_no']        = getModelColumnById(SpDrivers, driver_details.driver_id, 'primary_contact_number')
    context['user_list']                = user_list
    context['message']                  = 'Success'
    context['response_code']            = HTTP_200_OK
    return Response(context, status=HTTP_200_OK) 
# send otp to transporter for dispatch crates
@csrf_exempt
@api_view(["POST"])
def sendOtp(request):
    ...
    if user_details:
        ...
        user_otp.order_id       = user_details.id
        user_otp.otp            = otp
        user_otp.save()
...
def dispatchCrates(request):
    ...
    heading     = 'Crates has been dispatched from plant'
    activity    = 'Crates has been dispatched from plant by '+user_name+' to '+transporter_details+' on '+datetime.now().strftime('%d/%m/%Y | %I:%M %p') 
    saveActivity('Crate Management', 'Crate Dispatch', heading, activity, request.user.id, user_name, 'Orderplaced.png', '2', 'app.png')
    context = {}
    context['message']                  = 'Crates has been dispatched successfully.'
    context['response_code']            = HTTP_200_OK
    return Response(context, status=HTTP_200_OK)
