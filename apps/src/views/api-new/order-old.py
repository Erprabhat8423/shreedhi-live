...
def orderSchemeList(request):
    ...
    for scheme in scheme_list:
        free_scheme             = SpUserSchemes.objects.filter(scheme_type=1, user_id=request.data.get("user_id"), status=1).values('id', 'scheme_id', 'scheme_name', 'state_id', 'route_id', 'town_id', 'scheme_start_date', 'scheme_end_date', 'applied_on_variant_id', 'applied_on_variant_name', 'minimum_order_quantity', 'order_container_id', 'order_container_name', 'order_packaging_id', 'order_packaging_name', 'free_variant_id', 'free_variant_name', 'container_quantity', 'pouch_quantity') 
        for free in free_scheme:
            product_id             = getModelColumnById(SpProductVariants, free['free_variant_id'], 'product_id')
            free['container_type'] = getModelColumnById(SpProductVariants, free['free_variant_id'], 'container_name')
            free['variant_size'] = float(getModelColumnById(SpProductVariants, free['free_variant_id'], 'variant_size'))
            free['no_of_pouch'] = int(getModelColumnById(SpProductVariants, free['free_variant_id'], 'no_of_pouch'))
    ...
    flat_scheme  = SpUserFlatSchemes.objects.filter(user_id=request.data.get("user_id"), incentive_amount__gt=0, status=1).values('id', 'scheme_id', 'scheme_name', 'state_id', 'route_id', 'town_id', 'scheme_start_date', 'scheme_end_date', 'applied_on_variant_id', 'applied_on_variant_name', 'incentive_amount', 'unit_id', 'unit_name','product_class_id') 
    for flat in flat_scheme:
        flat['no_of_pouch'] = getModelColumnById(SpProductVariants, flat['applied_on_variant_id'], 'no_of_pouch')
        flat['variant_size'] = getModelColumnById(SpProductVariants, flat['applied_on_variant_id'], 'variant_size')
