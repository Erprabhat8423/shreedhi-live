            ...
            if vehicle.password:
                if check_password(received_data['password'], vehicle.password):
                    if vehicle.route_id is not None :
                        ...
                        response['route_id'] = vehicle.route_id
                        response['api_key'] = vehicle.api_token
                        response['is_route'] = vehicle.is_route
                        if (vehicle.is_route == 0) :
                            main_route=SpMainRoutes.objects.filter(id=vehicle.route_id).values("sub_route").first()
                            if main_route['sub_route'] is not None:
                                sub_route_id=(main_route['sub_route']).split(",")
                                routes = list(SpRoutes.objects.filter(id__in=sub_route_id).values())
                                for route in routes:
                                    try:
                                        driver_detail = SpVehicles.objects.filter(route_id=route['id'],is_route=1).first()
                                    except SpVehicles.DoesNotExist:
                                        driver_detail = None
                                    
                                    if driver_detail is None:
                                        route['vehicle_id']               = None
                                        route['vehicle_no']               = None
                                        route['driver_id']                = None
                                        route['driver_name']              = None
                                        route['driver_contact_no']        = None
                                    else:
                                        route['vehicle_id']               = driver_detail.id
                                        route['vehicle_no']               = driver_detail.registration_number
                                        route['driver_id']                = driver_detail.driver_id
                                        route['driver_name']              = driver_detail.driver_name
                                        route['driver_contact_no']        = "-" if (driver_detail.driver_id is None) else getModelColumnById(SpDrivers,driver_detail.driver_id,'primary_contact_number')
                                        
                                response['sub_route']=list(routes)
                        else:
                            response['sub_route']=[]
...
@validateGET
@validate_logistic_api
def todayPrimaryTransportOrders(request):
    ...
    if 'route_id' in request.GET and request.GET['route_id'] == "":
        ...
        for user in user_list:
            if getModelColumnById(SpUsers, user['user_id'], 'user_type') == 2:
                ...
                route_users['dispatch_normal_crate']  = 0
                route_users['dispatch_jumbo_crate']  = 0
                route_users['store_name']         = getModelColumnById(SpUsers, user['user_id'], 'store_name')
                try:
                    order = SpOrders.objects.filter(user_id=user['user_id'],order_date__icontains=today.strftime("%Y-%m-%d"),route_id=route_id,order_status__gt=2,dispatch_order_status__gt=0).values('id','user_id', 'order_code','town_name','route_name','user_name', 'order_date', 'order_status', 'order_total_amount', 'order_items_count','dispatch_order_status').first()
                except SpOrders.DoesNotExist:
                    order = None
                
                if order:
                    today   = given_order_date
                    if SpTransporterCrateLedger.objects.filter(user_id=order['user_id'],updated_datetime__icontains=today.strftime("%Y-%m-%d"),normal_debit=0,jumbo_debit=0).exists():
                        transport_crates=SpTransporterCrateLedger.objects.filter(user_id=order['user_id'],updated_datetime__icontains=today.strftime("%Y-%m-%d"),normal_debit=0,jumbo_debit=0).first()
                        ...
                        route_users['dispatch_order_status']       = order['dispatch_order_status']
                        route_users['order_items_count']  = order['order_items_count']
                        
                        route_users['dispatch_normal_crate'] = transport_crates.normal_credit
                        route_users['dispatch_jumbo_crate'] = transport_crates.jumbo_credit
...
@validateGET
@validate_logistic_api
def todaySecondaryTransportOrders(request):
    if 'date' in request.GET and request.GET['date'] != "":
        given_order_date = datetime.strptime(request.GET['date'], '%d/%m/%Y')
    else:
        given_order_date = datetime.now()
    ...
    if vehicle.route_id is None:
        ...
        for user in user_list:
            if getModelColumnById(SpUsers, user['user_id'], 'user_type') == 2:
                ...
                route_users['store_name']         = getModelColumnById(SpUsers, user['user_id'], 'store_name')
                try:
                    order = SpOrders.objects.filter(user_id=user['user_id'],order_date__icontains=today.strftime("%Y-%m-%d"),route_id=route_id,order_status__gt=2,dispatch_order_status=2).values('id','user_id', 'order_code','town_name','route_name','user_name', 'order_date', 'order_status', 'order_total_amount', 'order_items_count').first()
                except SpOrders.DoesNotExist:
                    order = None
...
def sendApprovalRequest(request):
    ...
    if status_code == 200 :
        ...
        if vehicle.route_id is None:
            ...
            try:
                crate = SpTransporterCrateLedger.objects.filter(user_id=received_data['user_id'],updated_datetime__icontains=today.strftime("%Y-%m-%d")).first()
                ...
                if order.order_status < 4 :
                    if SpOrderOtp.objects.filter(order_id=received_data['order_id'],otp=received_data['otp'],vehicle_id=vehicle.id).exists():
                        ...
                        if order.id :
                            ...
                            item.is_route                   = getModelColumnById(SpVehicles, vehicle.id, 'is_route')
                            item.updated_datetime           = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                            item.save()
                            ...
                            item.jumbo_debit                = 0
                            item.jumbo_balance              = jumbo_balance 
                            item.is_route                   = getModelColumnById(SpVehicles, request.data.get("vehicle_id"), 'is_route')
                            item.updated_datetime           = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                            item.save()    
...
@validatePOST
@validate_logistic_api
def sendTransferOtp(request):
    response={}
    received_data=json.loads(request.body)
    if 'dispatch_normal_crates' not in received_data or received_data['dispatch_normal_crates'] == "" :
        response['message'] = "Please provide dispatch normal crate"
        status_code = 404
        JsonResponse(response,status = status_code)
        
    if 'dispatch_jumbo_crates' not in received_data or received_data['dispatch_jumbo_crates'] == "" :
        response['message'] = "Please provide dispatch jumbo crate"
        status_code = 404
        JsonResponse(response,status = status_code)
    
    if 'normal_crates' not in received_data or received_data['normal_crates'] == "" :
        response['message'] = "Please provide normal crate"
        status_code = 404
        JsonResponse(response,status = status_code)
    
    if 'jumbo_crates' not in received_data or received_data['jumbo_crates'] == "" :
        response['message'] = "Please provide jumbo crate"
        status_code = 404
        JsonResponse(response,status = status_code)
    
    if received_data['dispatch_jumbo_crates'] == '0' and received_data['dispatch_normal_crates'] == '0' and received_data['jumbo_crates'] == '0' and received_data['normal_crates'] == '0' :
        response['message'] = "Please provide crates quantity."
        status_code = 404
        JsonResponse(response,status = status_code)
    api_token = request.headers['Authorization']
    user_id = getModelColumnByColumnId(SpVehicles,'api_token',api_token,'id')
    try:
        user_details = SpDrivers.objects.filter(primary_contact_number=received_data["mobile_no"], status=1).first()
    except SpUsers.DoesNotExist:
        user_details = None
    try:
        vehicle_details = SpVehicles.objects.filter(id=received_data["vehicle_id"], status=1).first()
    except SpVehicles.DoesNotExist:
        vehicle_details = None
    
    if not vehicle_details:
        return JsonResponse({'message': 'Invalid vehicle id', 'response_code': 400}, status=400) 
    if user_details:
        otp = generateOTP()
        try:
            user_otp = SpOrderOtp.objects.filter(vehicle_id=received_data["vehicle_id"]).first()
        except SpOrderOtp.DoesNotExist:
            user_otp = None
        if user_otp:
            SpOrderOtp.objects.filter(vehicle_id=received_data["vehicle_id"]).delete()
        user_otp                = SpOrderOtp()
        user_otp.vehicle_id     = received_data["vehicle_id"]
        user_otp.order_id       = user_id
        user_otp.otp            = otp
        user_otp.save()

        user_name   = str(getModelColumnById(SpVehicles,received_data["vehicle_id"], 'driver_name'))
        
        message = ""
        if (int(received_data["dispatch_normal_crates"]) > 0  or int(received_data["dispatch_jumbo_crates"]) > 0 ): 
            message += "You are receiving "
            
        if int(received_data["dispatch_normal_crates"]) > 0 :
            message += str(int(received_data["dispatch_normal_crates"])) + " normal crate"
        if int(received_data["dispatch_jumbo_crates"]) > 0 :
            if message != "You are receiving ":
                message += " and "
            message += str(int(received_data["dispatch_jumbo_crates"])) + " jumbo crate"
                
        if (int(received_data["normal_crates"]) > 0  or int(received_data["jumbo_crates"]) > 0 ): 
            if message != "":
                message += " and "
            message += "You are giving "
        
        if int(received_data["normal_crates"]) > 0 :
            message += str(int(received_data["normal_crates"])) + " normal crate"
        if int(received_data["jumbo_crates"]) > 0 :
            if int(received_data["normal_crates"]) > 0:
                message += " and "
            message += str(int(received_data["jumbo_crates"])) + " jumbo crate"  
        
        
        message += ". Share this OTP("+ str(otp) +") with "+str(user_name)+" for transfer ordered crates."
        message_title   = "Transfer Order OTP"
        title           = 'Transfer Order Crates'
    
        # sendSMS('SAAHAJ',received_data["mobile_no"],message)
        #-----------------------------notify android block-------------------------------#
        notification_image = ""
        
        message_body = message
        userFirebaseToken = getModelColumnById(SpVehicles,received_data["vehicle_id"],'firebase_token')
        registration_number = getModelColumnById(SpVehicles, received_data["vehicle_id"], 'registration_number')
        transporter_details = getModelColumnById(SpVehicles, received_data["vehicle_id"], 'driver_name')+'('+registration_number+')'
        ...
        if userFirebaseToken is not None and userFirebaseToken != "" :
            ...
            #-----------------------------notify android block-------------------------------#
        #-----------------------------save notification block----------------------------#
        saveNotification(None,None,'Order Management',title,message_title,message_body,notification_image,received_data["vehicle_id"],user_name,received_data["vehicle_id"],transporter_details,'order.png',2,'app.png',1,2)
        #-----------------------------save notification block-------- --------------------#
        context = {}
        ...
        return JsonResponse({'message': 'Invalid mobile no.', 'response_code': 400}, status=400)  



# Transfer crates to Party
   
@csrf_exempt
@validatePOST
@validate_logistic_api
def transferOrdertoOtherParty(request):
    received_data=json.loads(request.body)
        
    if received_data["user_id"] is None or received_data["user_id"] == '':
        return JsonResponse({'message': 'User Id field is required', 'response_code': 400}, status=400)
    if received_data["otp"] is None or received_data["otp"] == '':
        return JsonResponse({'message': 'OTP is required', 'response_code': 400}, status=400)
    if received_data["vehicle_id"] is None or received_data["vehicle_id"] == '':
        return JsonResponse({'message': 'Vehicle Id is required', 'response_code': 400}, status=400)  
    if not SpVehicles.objects.filter(id=received_data["vehicle_id"]).exists():
        return JsonResponse({'message': 'Invalid Vehicle Id', 'response_code': 400}, status=400)
    if not SpOrderOtp.objects.filter(vehicle_id=received_data["vehicle_id"], order_id=received_data["user_id"],otp=received_data["otp"]).exists():
        return JsonResponse({'message': 'Invalid OTP', 'response_code': 400}, status=400)        
    if received_data["dispatch_crates_list"]  == '' and received_data["recieve_crates_list"] == '':
        return JsonResponse({'message': 'Crate Details is required', 'response_code': 400}, status=400)  
        
        
    ###---- Dispatch Crates ----####
    dispatch_crates_list = received_data['dispatch_crates_list']
    
    if dispatch_crates_list:
        for crate in dispatch_crates_list:
            # Decrease Quantity From MAin Route Transporter
            if int(crate['normal_crate']) >0 or int(crate['jumbo_crate']) >0:
                if int(getModelColumnById(SpOrders,int(crate['order_id']),'dispatch_order_status')) == 1:
                    if not SpTransporterCrateLedger.objects.filter(transporter_id=received_data["user_id"], user_id=crate['ss_distributor_id']).exists():
                        normal_balance = crate['normal_crate']
                        jumbo_balance = crate['jumbo_crate']
                    else:
                        last_row = SpTransporterCrateLedger.objects.filter(transporter_id=received_data["user_id"], user_id=crate['ss_distributor_id']).order_by('-id').first()
                    ...
                    item                            = SpTransporterCrateLedger()
                    item.plant_user_id              = None
                    item.transporter_id             = received_data["user_id"]
                    item.driver_id                  = getModelColumnById(SpVehicles, received_data["user_id"], 'driver_id')
                    item.driver_name                = getModelColumnById(SpVehicles, received_data["user_id"], 'driver_name')
                    ...
                    item.jumbo_balance              = jumbo_balance 
                    item.is_route                   = getModelColumnById(SpVehicles, received_data["user_id"], 'is_route')
                    item.updated_datetime           = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    item.save()
                
                    #save crates details
                    if not SpTransporterCrateLedger.objects.filter(transporter_id=received_data["vehicle_id"], user_id=crate['ss_distributor_id']).exists():
                        normal_balance = crate['normal_crate']
                        jumbo_balance = crate['jumbo_crate']
                    else:
                        last_row = SpTransporterCrateLedger.objects.filter(transporter_id=received_data["vehicle_id"], user_id=crate['ss_distributor_id']).order_by('-id').first()
                        normal_balance = int(last_row.normal_balance)+int(crate['normal_crate']) 
                        jumbo_balance = int(last_row.jumbo_balance)+int(crate['jumbo_crate'])
                    item                            = SpTransporterCrateLedger()
                    item.plant_user_id              = None
                    item.transporter_id             = received_data["vehicle_id"]
                    item.driver_id                  = getModelColumnById(SpVehicles, received_data["vehicle_id"], 'driver_id')
                    item.driver_name                = getModelColumnById(SpVehicles, received_data["vehicle_id"], 'driver_name')
                    item.user_id                    = crate['ss_distributor_id']
                    ...
                    item.jumbo_debit                = 0
                    item.jumbo_balance              = jumbo_balance 
                    item.is_route                   = getModelColumnById(SpVehicles, received_data["vehicle_id"], 'is_route')
                    item.updated_datetime           = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    item.save()     
                    
                    data = SpOrders.objects.get(id=crate["order_id"])
                    data.dispatch_order_status = 2
                    data.save() 
     
        
    ###---- Recieve Crates ----####
    recieve_crates_list = received_data['recieve_crates_list']
    if recieve_crates_list:
        for crate in recieve_crates_list:
            # Decrease Quantity From Sub Route Transporter
            if int(crate['normal_crate']) >0 or int(crate['jumbo_crate']) >0:
                if not SpTransporterCrateLedger.objects.filter(transporter_id=received_data["vehicle_id"], user_id=crate['ss_distributor_id']).exists():
                    normal_balance = crate['normal_crate']
                    jumbo_balance = crate['jumbo_crate']
                ...
                # Save Quantity For MAin Route Transporter
                if not SpTransporterCrateLedger.objects.filter(transporter_id=received_data["user_id"], user_id=crate['ss_distributor_id']).exists():
                    normal_balance = crate['normal_crate']
                    jumbo_balance = crate['jumbo_crate']
                else:
    ...
    SpOrderOtp.objects.filter(vehicle_id=received_data["vehicle_id"], order_id=received_data["user_id"],otp=received_data["otp"]).delete()  
        
    registration_number = getModelColumnById(SpVehicles, received_data["vehicle_id"], 'registration_number')
    transporter_details = getModelColumnById(SpVehicles, received_data["vehicle_id"], 'driver_name')+'('+registration_number+')'
    user_name   = str(getModelColumnById(SpVehicles,received_data["user_id"], 'driver_name'))
    heading     = 'Order has been transferred from '+(getModelColumnById(SpVehicles, received_data["user_id"], 'registration_number'))
    activity    = 'Order has been transferred from '+(getModelColumnById(SpVehicles, received_data["user_id"], 'registration_number'))+' by '+user_name+' to '+transporter_details+' on '+datetime.now().strftime('%d/%m/%Y | %I:%M %p') 
    saveActivity('Crate Management', 'Crate Transferred', heading, activity, received_data["user_id"], user_name, 'Orderplaced.png', '2', 'app.png')
    context = {}
    context['message']                  = 'Order has been Transferred successfully.'
    context['response_code']            = 200
    return JsonResponse(context, status=200)



# Recieve crates from Party
@csrf_exempt
@validatePOST
@validate_logistic_api
def receiveCratesfromOtherParty(request):
    ...
    if not SpVehicles.objects.filter(id=received_data["vehicle_id"]).exists():
        return JsonResponse({'message': 'Invalid Vehicle Id', 'response_code': 400}, status=400)
    if not SpOrderOtp.objects.filter(vehicle_id=received_data["vehicle_id"], order_id=getModelColumnById(SpVehicles, received_data["vehicle_id"], 'driver_id'),otp=received_data["otp"]).exists():
        return JsonResponse({'message': 'Invalid OTP', 'response_code': 400}, status=400)
    #reset otp
    #SpOrderOtp.objects.filter(vehicle_id=received_data["vehicle_id"], order_id=getModelColumnById(SpVehicles, received_data["vehicle_id"], 'driver_id'),otp=received_data["otp"]).delete()         
    if received_data["crates_list"] is None or received_data["crates_list"] == '':
        return JsonResponse({'message': 'Crate Details is required', 'response_code': 400}, status=400)  
          
     
    
    crates_list = received_data['crates_list']
    ...
    transporter_details = getModelColumnById(SpVehicles, received_data["vehicle_id"], 'driver_name')+'('+registration_number+')'
    user_name   = str(getModelColumnById(SpVehicles,received_data["user_id"], 'driver_name'))
    heading     = 'Crates has been received by '+user_name
    activity    = 'Crates has been received by '+user_name+' from '+transporter_details+' on '+datetime.now().strftime('%d/%m/%Y | %I:%M %p') 
    saveActivity('Crate Management', 'Crate Received', heading, activity, received_data["user_id"], user_name, 'Orderplaced.png', '2', 'app.png')
    context = {}
